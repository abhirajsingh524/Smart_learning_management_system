...
function initChart(data) {
  ...
  labChart = new Chart(ctx, {
    ...
    data: {
      ...
      datasets: [
        { label: "Train", data: data.map(d => d.train), borderColor: "#6C63FF", borderWidth: 2, fill: false, tension: 0.4, pointRadius: 3, pointBackgroundColor: "#6C63FF" },
        { label: "Val",   data: data.map(d => d.val),   borderColor: "#00D4AA", borderWidth: 2, fill: false, tension: 0.4, pointRadius: 3, pointBackgroundColor: "#00D4AA", borderDash: [4,2] }
      ]
    },
    options: {
      ...
      scales: {
        x: { ticks: { color: "#94A3B8", font: { size: 11 } }, grid: { color: "rgba(108,99,255,0.1)" } },
        y: { min: 0.4, max: 1, ticks: { color: "#94A3B8", font: { size: 11 } }, grid: { color: "rgba(108,99,255,0.1)" } }
      }
    }
  });
}
...
async function runExperiment() {
  ...
  initChart(d.training_curve);

  box.style.display = "block";
  box.innerHTML = `
    <div style="font-size:12px;color:var(--accent);font-weight:600;margin-bottom:8px">✓ Training Complete</div>
